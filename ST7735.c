...
void ST7735_uBinOut6(uint32_t n){
  ...
  uint32_t t = ((n*100) +32) >> 6; // divide input by 64
...
void ST7735_XYplotInit(char *title, int32_t minX, int32_t maxX, int32_t minY, int32_t maxY, uint16_t color){
	...
	ST7735_FillRect(0, 32, 127, 159,color);
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[], uint16_t color){
		...
		uint32_t i = 0;			//index of the buffers
		uint32_t tmpX = 0;	//temp x-coordinate to plot
		uint32_t tmpY = 0;	//temp y-coordinate to plot
	
		for(i =0; i < num; i ++){	//draw each character at a position
			tmpX = (127*(bufX[i]-MiniX))/(MaxiX-MiniX);
			tmpY = 32+(127*(MaxiY-bufY[i]))/(MaxiY-MiniY);
			
			ST7735_DrawPixel(tmpX,   tmpY,   color);
			ST7735_DrawPixel(tmpX+1, tmpY,   color);
			ST7735_DrawPixel(tmpX,   tmpY+1, color);
			ST7735_DrawPixel(tmpX+1, tmpY+1, color);
